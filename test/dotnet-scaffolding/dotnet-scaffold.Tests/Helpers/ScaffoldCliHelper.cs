...
internal static class ScaffoldCliHelper
{
    ...
    /// dotnet installation directory. Prepends the dotnet directory to PATH so
    /// that any grandchild process (such as the MSBuild SDK resolver or
    /// DotnetCliRunner) also finds the correct dotnet executable. Clears
    ...
    private static void ConfigureDotNetEnvironment(ProcessStartInfo startInfo, string? targetFramework = null)
    {
        ...
        if (dotnetPath != "dotnet")
        {
            ...
            // Prepend the dotnet directory to PATH so that child processes spawned
            // by the CLI (e.g., MSBuild SDK resolvers, DotnetCliRunner) also
            // resolve the correct dotnet for the target SDK.  Without this, the
            // system-default dotnet may not know about preview SDK versions like
            // net11.0 and the MSBuildWorkspace SDK resolver will fail silently,
            // producing an empty compilation and a "model returns null" failure.
            ...
            var pathEntries = existingPath.Split(Path.PathSeparator);
            if (!pathEntries.Any(d => string.Equals(d, dotnetRoot, System.StringComparison.OrdinalIgnoreCase)))
            {
                startInfo.Environment["PATH"] = dotnetRoot + Path.PathSeparator + existingPath;
            }
        }
